...
3. **滾動窗口。** 規則係 **任何 180 日內** 不超過 180 日，唔係「每年」
   180 日。即係話 11 月去 100 日加 1 月再去 90 日，雖然兩個年度都冇超
   180 日，但係喺呢個滾動窗口入面已經爆咗。
...
- **儀表板** — 一眼睇晒每位家人嘅狀態：目前 180 日窗口期內離港咗幾多日、
...
| 規則 | 邊度處理 | 備註 |
|------|---------|------|
| 180 日滾動窗口 | `calcRollingWindow()` | 可以以任何日期作為參考點，唔限於今日 |
| 午夜規則 | `tripAbsenceRange()` / `tripAbsenceDays()` | 出發日計入，返港日不計 |
| 5 年 ILR 申請期 | `ilrEarliest` 計算 | `BNO 批准日 + 5 年` |
| 每 6 個月最差窗口檢查 | `worstWindow` 迴圈 | 揾出由批准日起最嚴重嘅 6 個月段 |
